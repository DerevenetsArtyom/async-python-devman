"""
Модуль для работы с препятствиями.
Помогает вести учет препятствия и отображать на карте место их расположения.

## Как создать препятствия
obstacles = [
...
for obstacle in obstacles:
    print(obstacle.row, obstacle.column,
          obstacle.rows_size, obstacle.columns_size, obstacle.uid)

## Как вывести препятствия на экран
obstacles = [
    Obstacle(10, 10),  # первое препятствие
    Obstacle(10, 12, uid='второе препятствие с названием'),
    Obstacle(20, 20, 5, 5, uid='третье большое препятствие'),
...
# TODO передать управление корутине coroutine

### Как проверить столкновение с препятствием
Функция def has_collision определяет факт столкновения предметов.
На вход принимает координаты объектов и их габариты, возвращает True или False.
Пример использования:

has_collision(
    (10, 10),   # верхний левый угол препятствия
    (2, 2),     # размер препятствия
    (11, 11),   # верхний левый угол вторго объекта
)
# вернет True
Если уже создан объект Obstacle, то можно воспользоваться его методом:

obstacle = Obstacle(10, 10, 2, 2)
obstacle.has_collision(11, 11)  # returns True
"""
import asyncio
